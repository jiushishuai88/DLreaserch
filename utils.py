...
def get_data_loader(transform_train, transform_test, config):
    ...
    if config.dataset == 'cifar10':
        # cifar10数据集
        trainset = torchvision.datasets.CIFAR10(root=config.data_path, train=True, download=True,
                                                transform=transform_train)
        testset = torchvision.datasets.CIFAR10(root=config.data_path, train=False, download=True,
                                               transform=transform_test)
    elif config.dataset == 'cifar100':
        # cifar100数据集
        trainset = torchvision.datasets.CIFAR100(root=config.data_path, train=True, download=True,
                                                 transform=transform_train)

        testset = torchvision.datasets.CIFAR100(root=config.data_path, train=False, download=True,
                                                transform=transform_train)
    logging.info("==============data-file:{}==============".format(config.data_path))
...
def mixup_data(x, y, alpha, device):
    if alpha > 0:
        ...
        lam = 1
    batch_size = x.size()[0]
    index = torch.randomperm(batch_size).to(device)
