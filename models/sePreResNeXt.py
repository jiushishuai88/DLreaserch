...
class SePreResneXt(nn.Module):
    ...
    def _make_layer(self, block, channels, blocks,index = 1, stride=1):  # 16 10
        ...
        layers = [block(self.in_channels, channels,self.cardinality,self.base_width ,index = index,stride=stride,downsample=downsample)]
...
def preresneXt29_8x64d(num_class):
    return SePreResneXt(depth=29, num_classes=num_class,cardinality=8,base_width=64)


def preresneXt29_16x64d(num_class):
    return SePreResneXt(depth=29, num_classes=num_class,cardinality=16, base_width=64)

def preresneXt29_32x32d(num_class):
    return SePreResneXt(depth=29, num_classes=num_class,cardinality=32, base_width=32)

def preresneXt29_32x16d(num_class):
    return SePreResneXt(depth=29, num_classes=num_class,cardinality=32, base_width=16)

def se_preresneXt29_32x8d(num_class):
    return SePreResneXt(depth=29, num_classes=num_class,cardinality=32, base_width=8)